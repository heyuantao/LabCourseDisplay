...
class LoginPage extends React.Component {
    ...
    validateFormField() {
        ...
        if (!formData.get("captcha")) {
            this.setState({ formFieldValidateInfo: "请输入验证码 ！" })
            return -1
        }
    ...
    render() {
        ...
        //const isLogin = this.props.user.get("isLogin");
        const isLogin = false;
        const user = fromJS({"username":"abc"});
        const formData = fromJS({"email":"test@abc.com"});
        return (
...
const mapDispatchToProps = (dispatch) => {
    return{
        login(username,password,captcha){
            //dispatch(UserActionCreator.login(username,password))
        },
        fetchUser(){
